...
export default function RecruitmentsPage() {
    ...
    useEffect(() => {
        if (isAdmin && formTextareaRef.current) {
            ...
            formTextareaRef.current.style.height = `${formTextareaRef.current.scrollHeight + 20
                }px`;
        }
    }, [isAdmin, clubData]);
    ...
    const handleDeleteSkillTag = (skill) => {
        if (confirm("Are you sure you want to delete this link?") === false)
            return;
    ...
    let relevantInfo = isAdmin ? (
        <>
            ...
            <button
                ...
                className="save-info text"
            >
                Save Recruitment Info
            </button>
        </>
    ) : (
    ...
    let FormLink = isAdmin ? (
        ...
        <>
            <div>
                <a href={clubData.recruitment_form} target="_blank">Form Link</a>
            </div>
